...
class ChessClassifierPipeline2:
    ...
    def __init__(self, data_path='data/stat_parties.parquet'):        
        ...
        available_classes = self.df['class'].unique()
        self.custom_order = [c for c in self.custom_order if c in available_classes]
    ...
    def prepare_features(self):
        """
        Version ultra-robuste pour éviter les problèmes de 'shape mismatch'
        """
        print("\n🔧 PRÉPARATION DES FEATURES (Style & Répertoire)")
        print("="*70)
        
        feature_list = []
        
        # On s'assure d'utiliser l'index d'origine pour tout aligner
        main_index = self.df.index

        # 1. Temps moyen
        if 'avg_opening_move_time' in self.df.columns:
            # On force l'index pour être sûr
            time_feat = self.df[['avg_opening_move_time']].copy()
            feature_list.append(time_feat)
        
        # 2. Répertoire (Familles d'ouvertures)
        print("📊 Analyse du répertoire (Familles d'ouvertures)...")
        top5_families = self._extract_opening_frequencies(
            self.df['top_5_openings_used'], 
            prefix='style'
        )
        # On force l'index du DataFrame généré pour qu'il colle à self.df
        top5_families.index = main_index
        feature_list.append(top5_families)
        
        # 3. Features agrégées
        print("🧮 Calcul des indicateurs de style...")
        
        # Diversité
        diversity = self.df['top_5_openings_used'].apply(
        ...
        # 4. Le Concat Final
        # L'argument axis=1 fusionne les colonnes. 
        # Si une Series n'a pas de nom, Pandas peut râler, mais ici elles sont nommées (.rename).
        X = pd.concat(feature_list, axis=1)
        
        # Remplissage des vides et nettoyage
        X = X.fillna(0)
        
        # Label
        y = self.df['class'].apply(lambda x: self.custom_order.index(x))
    ...
    def plot_confusion_matrix(self, y_true, y_pred):
        ...
        plt.tight_layout()
        plt.savefig('confusion_matrice3.png', dpi=300, bbox_inches='tight')
        plt.show()
        
        print("✅ Matrice sauvegardée: confusion_matrice3.png")
    
    def plot_feature_importance(self, top_n=20):
        """Affiche l'importance des features (version sécurisée)"""
        ...
        # SÉCURITÉ : On ne peut pas afficher plus de features qu'on en a
        ...
        plt.title(f'Top {actual_top_n} Features les plus importantes', fontsize=16)
        
        # On s'assure que les deux listes font la même taille : actual_top_n
        plt.barh(
            range(actual_top_n),
            importances[indices],
            align='center'
        ...
        print(f"✅ Graphique sauvegardé (basé sur {actual_top_n} features)")
...
if __name__ == "__main__":
    ...
    print("   - confusion_matrice3.png")
    print("   - feature_importance3.png")
